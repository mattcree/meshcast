...
- [ ] **Batch A — correctness & security core (S each)**: R1 rejoin, S1 PIN oracle, S2 single-use token + controller id, S3 relay-only addrs, S5 authorship checks, S6 dirs/notify, R2 outbox + bot connectivity, R4 card-post failure, R5 timeouts, R7/C2 bot task abort, R10 link check, R12 online timeout, O9 redact ticket, R8 tray atomic write.
- [ ] **Batch B — operability (M)**: O1 log files + panic hook, O4 filters, O2 versions + `Signal::Hello` + bot `--version` + app footer + `status`, O3 installer smoke + runtime deps + desktop fallback, O5 config backup, R11 error_at, O8 upgrade guard.
- [ ] **Batch C — performance (S/M)**: F2 capture fps, F1 hardware encoder with fallback + `video.codec`, F4 decoder Auto, F5 coalescing + never-drop presses, F6 repaint cadence, F3 docs + 1080p60 warning, F8 features/timeouts/no-fsync.
- [ ] **Batch D — UX (S each)**: U1 card layout, U2 consent dialog (what/why/countdown, mic toggle, mic default off), U3 vocabulary, U4 Windows, U5 failure copy, U6 one link per bot, U7 timeouts, U8 control prompt identity, U9 watch page platform-aware, U10 selected polish.
