...
| Link buttons | Only `http(s)://` (and `discord://`) | A `meshcast://watch/<ticket>` button is rejected. Either proxy through an https page, or route the click back through the bot. Meshcast routes through the bot (see §4.4); an https redirect page is a backlog item for viewers who haven't linked. |
